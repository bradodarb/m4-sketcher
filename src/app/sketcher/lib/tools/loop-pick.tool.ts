...
import { SketchObject, EndPoint, Arc } from '../geometry/render-models';
...
export class LoopPickTool extends Tool {
  ...
  public loops: Map<any, any>;
  public marked: Set<SketchObject>;
  public pointToObject: Map<any, any>;
  public pickedLoop;
  ...
  clearMarked() {
    const items = this.marked.values();
    for (var obj = items.next().value; obj = items.next().value;) {
      obj.marked = null;
    }
    this.marked.clear();
  }
  ...
  reindexLoops() {
    ...
    this.viewer.accept((obj) => {
      if (obj.className == 'M4CAD.TWO.EndPoint' && obj.parent &&
